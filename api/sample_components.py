...
def get_sample_component_by_id(db, sample_component_id):
    return db.sample_components.find_one({"_id": ObjectId(sample_component_id)})

def find_sample_component_ids_by_sample_id(db, sample_component_id):
...
def delete_sample_component_by_id(db, sample_component_id):
    if isinstance(sample_component_id, ObjectId):
